...
{
    public static void Create(String url, String message) throws Exception
    {
        ...
        // Configurar o editor
        Font font = new Font(Font.MONOSPACED, Font.BOLD, (int)Math.round(width / message.length() * 1.56));
        pixels.setFont(font);
        ...
        transform.translate((int)Math.round(width / 48), height_new - (int)Math.round(height_new * 0.025));

        // Fazer alterações
        pixels.drawImage(img, 0, 0, null);
        pixels.setColor(Color.RED);
        pixels.drawString(message, (int)Math.round(width / 48), height_new - (int)Math.round(height_new * 0.025));
        ...
        ImageIO.write(sticker, "png", new File("stickers/" + message.replace(":", "-") + ".png"));
    }
}
